package net.grinner117.radiantlibary.networking;
...
public class PacketUpdateFlight {
		...
		public void handle(Supplier<NetworkEvent.Context> ctx) {
			ctx.get().enqueueWork(() -> {
				RadiantLibary.proxy.getPlayer().getAbilities().mayfly = canFly;
				RadiantLibary.proxy.getPlayer().getAbilities().flying = wasFlying;
			});
